...
import { StyleSheet, Text, ScrollView, KeyboardAvoidingView, NativeModules, NativeEventEmitter, TextInput, Button, Alert, Linking } from 'react-native';
...
export default function Demo() {
  const [money, setMoney] = React.useState('70000')
  const [token, setToken] = React.useState('')
  const [returncode, setReturnCode] = React.useState('')
  const [order, setOrder] = React.useState(null)
  const [link, setLink] = React.useState('')
  const [resJson, setResJson] = React.useState(null)
  ...
  function payOrder() {
    ...
    }).then(response => response.json()).then(res => console.log(res.returnCode)).catch(err => console.log(err))
  }
      ...
      <KeyboardAvoidingView style={styles.container}>
        ...
        {returncode == 1 ?
          <Button
            ...
            type="outline"
            onPress={() => { payOrder() }}
          /> : returncode == -1 ? <Text>CÓ lỗi giao dịch</Text> : returncode == 4 && <Text>Đã huỷ</Text>
        }
